...
void TileLayer::render(Window & window, sf::Vector2i levelSize) const 
{
...
{
	Level level;
	std::vector<sf::Vector2i> turretPlacementPositions;
		...
		level.m_soilderSpawnRate, level.m_tankSpawnRate, level.m_planeSpawnRate))
	{
		...
		{
			level.m_turrets.emplace_back(sf::Vector2f(position.x, position.y));
		}
...
void Level::addTurretAtPosition(sf::Vector2f position, eTurretType turretType, int& playerScore)
{
