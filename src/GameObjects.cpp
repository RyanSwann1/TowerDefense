...
//Projectile
Projectile::Projectile(sf::Vector2f startingPosition, sf::Vector2f startingDirection, eProjectileSender sentFrom, 
...
//Turret
Turret::Turret(sf::Vector2f startingPosition)
	: m_position(startingPosition),
	m_baseSprite(),
	m_headSprite(),
...
void Turret::setTurret(eTurretType turretType, sf::Vector2f position)
{
	m_active = true;
...
bool Turret::fire(const std::vector<Unit>& units, std::vector<Projectile>& projectiles) const
{
	for (const auto& unit : units)
	{
		if (Math::isWithinRange(m_position, unit.getPosition(), m_attackRange))
		{
			...
			sf::Vector2f dir = Math::getDirection(m_position, { unit.getPosition().x + xOffset, unit.getPosition().y });
...
//Unit
Unit::Unit(int baseTileID, int headTileID, const std::vector<sf::Vector2i>& movementPath, eUnitType unitType, eGameDifficulty gameDifficulty)
	...
	m_damage(0)
{
	...
	m_moveDirection = Math::getDirection(m_position, 
		sf::Vector2f(m_movementPath.back().x, m_movementPath.back().y));
}

sf::Vector2f Unit::getMoveDirection() const
{
...
void Unit::update(float deltaTime, const std::vector<Turret>& turrets, std::vector<Projectile>& projectiles)
{
	...
	if (m_unitType == eUnitType::Aircraft)
	{
		...
		sf::Vector2f newBasePosition(m_position.x + tileSize, m_position.y + tileSize);
	...
	{
		m_position = sf::Vector2f(m_movementPath.back().x, m_movementPath.back().y);
		m_baseSprite.setPosition(m_position);
		m_headSprite.setPosition(m_position);
		m_movementPath.pop_back();
		...
		m_moveDirection = Math::getDirection(m_position, sf::Vector2f(m_movementPath.back().x, m_movementPath.back().y));
	}
}
...
bool Unit::fire(const std::vector<Turret>& turrets, std::vector<Projectile>& projectiles) const
{
	for (const auto& turret : turrets)
	{
		...
		{
			sf::Vector2f dir = Math::getDirection(m_position, turret.getPosition());
